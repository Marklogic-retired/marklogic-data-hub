...
public class StepRangeFacetHandler implements FacetHandler {

    @Override
    public StructuredQueryDefinition buildQuery(DocSearchQueryInfo.FacetData data, StructuredQueryBuilder queryBuilder) {
        return queryBuilder
                .wordConstraint(Constants.RAN_STEP_WORD_CONSTRAINT_NAME, data.getStringValues().toArray(new String[0]));
    }
}
