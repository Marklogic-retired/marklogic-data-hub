...
describe("Validate Match warnings", () => {
  ...
  it("Navigate to match step and validate warning messages", () => {
    ...
    curatePage.alertMessage().eq(0).should("have.text", "Warning: Target Collections includes the source collection mapPersonJSON");
    curatePage.alertDescription().eq(0).should("have.text", "Please remove source collection from target collections");
    curatePage.targetCollection("Person");
    cy.wait(1000);
    cy.get("div[id=\"additionalColl\"]").type("{enter}");
    curatePage.saveSettings(matchStep).click();
    curatePage.alertMessage().eq(0).should("have.text", "Warning: Target Collections includes the target entity type Person");
    curatePage.alertDescription().eq(0).should("have.text", "Please remove target entity type from target collections");
    curatePage.alertMessage().eq(1).should("have.text", "Warning: Target Collections includes the source collection mapPersonJSON");
    curatePage.alertDescription().eq(1).should("have.text", "Please remove source collection from target collections");
  });
