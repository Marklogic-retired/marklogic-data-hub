...
async function editXPath(page) {
  ...
  await page.waitFor(1000);
  await page.waitForSelector(".ant-collapse > .ant-collapse-item:nth-child(9) > .ant-collapse-header > .anticon > svg");
  await page.click(".ant-collapse > .ant-collapse-item:nth-child(9) > .ant-collapse-header > .anticon > svg");

  await page.waitFor(1000);
  await page.waitForSelector("li > span > [class*=mapping-card_stepDetails] > [data-testid=map-Top-and-Bottom-Heavy-stepDetails]");
  await page.click("li > span > [class*=mapping-card_stepDetails] > [data-testid=map-Top-and-Bottom-Heavy-stepDetails]");
  await page.waitForSelector("#functionIcon");
  await page.waitForSelector(".ant-table-fixed > .ant-table-tbody > .ant-table-row-level-1 > .ant-table-column-has-actions > [class*=entity-map-table_expandIcon]");
  await page.click(".ant-table-fixed > .ant-table-tbody > .ant-table-row-level-1 > .ant-table-column-has-actions > [class*=entity-map-table_expandIcon]");
  ...
  await page.waitForSelector("#mapexpressionprop-100");
