...
const Overview: React.FC<Props> = props => {
  ...
  const homePageInfo = (
    <Popover id={`popover-overview`} className={styles.popoverHomePageInfo}>
      <Popover.Body className={styles.popoverHomePageInfoBody}>
        <div className={styles.homeInfoPopover} aria-label="homePageInfoPopover">
          {ToolbarBulbIconInfo.homePageInfo}
        </div>
      </Popover.Body>
    </Popover>
  );
  ...
  const focusCard = id => {
    if (cardRefMap[id].current !== null) {
      cardRefMap[id].current.focus();
    }
  };

  const cardKeyDownHandler = (event, id) => {
    if (event.key === "Enter" && props.enabled && props.enabled.includes(id)) {
      goToTile(id);
    }
                ...
                <span
                  ...
                  onKeyDown={event => {
                    if (event.key === "Enter" || event.key === " ") {
                      helpInfoViewChange(!helpInfoVisible, event);
                    } else if (event.key === "Tab" && helpInfoVisible) {
                      helpInfoViewChange(!helpInfoVisible);
                    }
                  }}
                  className={styles.helpInfoIcon}
                  aria-label="homePageInfoIcon"
                >
                  <img src={modelingInfoIcon} />
                </span>
              </OverlayTrigger>
            </span>
          }
        </div>
        <div className={styles.introText} aria-label={"introText"}>
          MarkLogic Data Hub Central makes it easy to manage your data. You can load, curate, and manage your data, or
          explore and export your data — all within Hub Central.
        </div>
      </div>
      <div className={styles.cardsContainer}>
        <div className={styles.cards}>
          <div
            className={getClassNames("load")}
            aria-label={"load-card"}
            ref={cardRefMap["load"]}
            tabIndex={0}
            onClick={() => {
              goToTile("load");
            }}
            onKeyDown={e => cardKeyDownHandler(e, "load")}
          >
            <div className={styles.head} />
            <div className={styles.subtitle}>
              <span className={styles.icon} aria-label="load-icon" />
              Load
            </div>
            <div className={styles.body}>
              Create and configure steps that define how data should be loaded.
              <div
                className={styles.docLink}
                tabIndex={0}
                onKeyDown={e => {
                  openDocumentation(e, "load");
                }}
              >
                <span
                  onClick={e => {
          ...
          <div
            className={getClassNames("model")}
            aria-label={"model-card"}
            ref={cardRefMap["model"]}
            tabIndex={0}
            onClick={() => {
              goToTile("model");
            }}
            onKeyDown={e => cardKeyDownHandler(e, "model")}
          >
            <div className={styles.head} />
            <div className={styles.subtitle}>
              <span className={styles.icon} aria-label="model-icon" />
              Model
            </div>
            <div className={styles.body}>
              Define the entity models that describe and standardize your data.
              <div
                className={styles.docLink}
                tabIndex={0}
                onKeyDown={e => {
                  openDocumentation(e, "model");
                }}
              >
                <span
                  onClick={e => {
          ...
          <div
            className={getClassNames("curate")}
            aria-label={"curate-card"}
            ref={cardRefMap["curate"]}
            tabIndex={0}
            onClick={() => {
              goToTile("curate");
            }}
            onKeyDown={e => cardKeyDownHandler(e, "curate")}
          >
            <div className={styles.head} />
            <div className={styles.subtitle}>
              <span className={styles.icon} aria-label="curate-icon" />
              Curate
            </div>
            <div className={styles.body}>
              Create and configure steps that curate and refine your data.
              <div
                className={styles.docLink}
                tabIndex={0}
                onKeyDown={e => {
                  openDocumentation(e, "curate");
                }}
              >
                <span
                  onClick={e => {
          ...
          <div
            ...
            onKeyDown={e => cardKeyDownHandler(e, "run")}
          >
            <div className={styles.head}>
              <div className={styles.subtitle}>
                <span className={styles.icon} aria-label="run-icon" />
                Run
              </div>
              <div className={styles.body}>
                {tiles.run.intro}
          ...
          <div
            className={getClassNames("explore")}
            aria-label={"explore-card"}
            ref={cardRefMap["explore"]}
            tabIndex={0}
            onClick={() => {
              goToTile("explore");
            }}
            onKeyDown={e => cardKeyDownHandler(e, "explore")}
          >
            <div className={styles.head}>
