...
import SaveQueriesDropdown, {
  SELECT_QUERY_PLACEHOLDER,
} from "@components/queries/saving/save-queries-dropdown/save-queries-dropdown";
...
const Query: React.FC<Props> = props => {
  ...
  const {searchOptions, setSearchOptions, applySaveQuery, clearAllGreyFacets, setSavedQueries, savedQueries} =
  ...
  const deleteConfirmation = (
    <HCModal show={deleteModalVisibility} onClick={() => setDeleteModalVisibility(false)}>
  ...
  const isSaveQueryChanged = () => {
    ...
    if (currentQuery && currentQuery.hasOwnProperty("savedQuery") && currentQuery.savedQuery.hasOwnProperty("query")) {
      ...
      if (
        (JSON.stringify(currentQuery.savedQuery.query.selectedFacets) !==
          JSON.stringify(searchOptions.selectedFacets) ||
          ...
          JSON.stringify(currentQuery.savedQuery.propertiesToDisplay) !==
            JSON.stringify(searchOptions.selectedTableProperties) ||
  ...
  useEffect(() => {
    if (
      searchOptions.nextEntityType === "All Entities" &&
  ...
  const resetIconClicked = () => {
    ...
    const resetQueryEditedConfirmation =
      ...
      searchOptions.selectedQuery !== SELECT_QUERY_PLACEHOLDER &&
      isSaveQueryChanged();
    const resetQueryNewConfirmation =
      props.isSavedQueryUser &&
      props.queries.length > 0 &&
      searchOptions.entityTypeIds.length > 0 &&
      (props.selectedFacets.length > 0 || searchOptions.query.length > 0 || searchOptions.sortOrder.length > 0) &&
      searchOptions.selectedQuery === SELECT_QUERY_PLACEHOLDER;
  ...
  useEffect(() => {
    if (Object.entries(currentQuery).length !== 0 && searchOptions.selectedQuery !== SELECT_QUERY_PLACEHOLDER) {
      ...
      setCurrentQueryName(
        currentQuery.hasOwnProperty("name") ? currentQuery["name"] : currentQuery["savedQuery"]["name"],
      );
      setCurrentQueryDescription(
        currentQuery.hasOwnProperty("description")
          ? currentQuery["description"]
          : currentQuery["savedQuery"]["description"],
      );
    } else {
      setCurrentQueryDescription("");
    }
          ...
          <div>
            <div className={styles.queryBar}>
              ...
              <div className={styles.iconBar}>
                <div>
                  {showSaveNewIcon && (
                    <span
                      ...
                      onKeyDown={event => {
                        if (event.key === "Enter") {
                          isEnabledSaveButton ? setOpenSaveModal(true) : setOpenSaveModal(false);
                        }
                      }}
                    >
                      <HCTooltip
                ...
                {searchOptions.selectedQuery !== SELECT_QUERY_PLACEHOLDER && <div>{ellipsisMenu}</div>}
                {deleteConfirmation}
                {props.isSavedQueryUser && showDiscardIcon && props.queries.length > 0 && (
                  <div>
            ...
            >
              <HCTooltip text={currentQueryDescription} id="current-query-description-tooltip" placement="top">
                <span>
                  {searchOptions.selectedQuery === SELECT_QUERY_PLACEHOLDER
                    ? ""
                    : searchOptions.selectedQuery &&
                      searchOptions.selectedQuery !== SELECT_QUERY_PLACEHOLDER &&
                      currentQueryDescription.length > 50
                      ? currentQueryDescription.substring(0, 50).concat("...")
            ...
            */}
              {searchOptions.selectedQuery === SELECT_QUERY_PLACEHOLDER ? (
                <HCTooltip
                  text={"To enable this action you should select a query"}
                  aria-label="clear-query-tooltip"
                  id="clear-query-tooltip"
                  placement="top"
                >
              ...
              <HCModal show={showResetQueryEditedConfirmation || showResetQueryNewConfirmation} onHide={onResetCancel}>
                <Modal.Header className={"bb-none"}>
                ...
                <Modal.Body className={"pt-0 px-4"}>
                  {showResetQueryEditedConfirmation && (
                    <div>
                      <p>
                        Your unsaved changes in the query <strong>{searchOptions.selectedQuery}</strong> will be lost.
                      </p>
                  ...
                  <div className={"d-flex justify-content-center mt-4 mb-2"}>
                    <HCButton
                      variant="outline-light"
                      key="back"
                      id="reset-confirmation-no-button"
                      className={"me-2"}
                      onClick={() => onNoResetClick()}
                    >
                      No
                    </HCButton>
                    <HCButton
                      key="submit"
                      id="reset-confirmation-yes-button"
                      variant="primary"
                      onClick={() => onResetOk()}
                    >
                      Yes
