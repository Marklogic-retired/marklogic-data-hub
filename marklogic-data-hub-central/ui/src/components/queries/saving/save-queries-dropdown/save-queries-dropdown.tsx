    ...
    <div>
      <Select
        ...
        formatOptionLabel={({value, label}) => {
          return <span data-cy={`query-option-${value}`}>{label}</span>;
        }}
      />
      <HCModal show={showConfirmation} onHide={onCancel}>
        <Modal.Header className={"bb-none"}>
          <button type="button" className="btn-close" aria-label="Close" onClick={onCancel} />
        </Modal.Header>
        <Modal.Body className={"pt-0 px-4"}>
          <p>
            <strong>{props.currentQueryName}</strong> has been edited since it was last saved.
          </p>
          <p>
            Would you like to save the changes to <strong>{props.currentQueryName}</strong> before switching to the new
            query
          </p>
          <div className={"d-flex justify-content-center"}>
            <HCButton
              ...
              key="back"
              className={"me-2"}
              id="query-confirmation-no-button"
              onClick={() => onNoClick()}
            >
              No
            </HCButton>
