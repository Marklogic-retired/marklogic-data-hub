...
test("Verify Start Time dropdown is rendered", () => {
  const {getByText} = render(<MonitorSidebar facets={{}} facetRender={jest.fn()} checkFacetRender={jest.fn()} />);
...
test("Verify Custom date picker is rendered", () => {
  const {getByText, getByPlaceholderText, getByLabelText} = render(
    <MonitorSidebar facets={{}} facetRender={jest.fn()} checkFacetRender={jest.fn()} />,
  );
