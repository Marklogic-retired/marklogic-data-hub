...
const ConfirmYesNo: React.FC<Props> = props => {
  return (
    <HCModal show={props.visible} size="sm" onHide={props.onNo}>
      <Modal.Body>
        <div aria-label="confirm-body" className={"text-center"}>
          {ConfirmYesNoMessages[props.type]}
        </div>
        <div className={"d-flex justify-content-center pt-4 pb-2"}>
          <HCButton
            className={"me-2"}
            variant="outline-light"
            aria-label={props.labelNo ? props.labelNo : "No"}
            onClick={props.onNo}
          >
            {props.labelNo ? (props.labelNo === "DiscardChangesNoButton" ? "No" : props.labelNo) : "No"}
          </HCButton>
          <HCButton
            aria-label={props.labelYes ? props.labelYes : "Yes"}
            variant="primary"
            type="submit"
            onClick={props.onYes}
          >
