...
function HCSider({
  ...
  const buttonPlacement = {
    [placement]: "100%",
    borderRadius: placement === "left" ? "0px 5px 5px 0px" : "5px 0px 0px 5px",
  };
  ...
  return (
    <div
      data-testid={`${identity ? identity + "-" : ""}hc-sider-component`}
      className={`${styles.siderContainer} ${containerClassName && containerClassName}`}
      style={HCSiderStyle}
    >
      <a
      ...
      >
        {icon}
        {showButtonLabel && button}
      </a>
      {open && (
        <div
          data-testid={`${identity ? identity + "-" : ""}hc-sider-content`}
          id={`${identity ? identity + "-" : ""}hc-sider-content`}
          className={`${styles.siderContentContainer} ${footer && styles.containerWithFooter} ${
