...
const DropDownWithSearch: React.FC<Props> = props => {
  ...
  const calcDropdownWidth = () => {
    if (indentList && srcData) {
      ...
      srcData.map(
        (element, index) =>
          (maxStringLengthInPx = Math.max(
            maxStringLengthInPx,
            element.value.length * charToPxScalingFactor + indentList[index],
          )),
      );
            ...
            return (
              <span data-testid={element.testId + "-option"} style={optionsStyle(element.index)} role={"option"}>
                {element.label}
                {
                  <HCTooltip
                    text="Multiple"
                    data-testid={element.testId + "Multiple-option-tooltip"}
                    id="multiple-option-tooltip"
                    placement="top"
                  >
