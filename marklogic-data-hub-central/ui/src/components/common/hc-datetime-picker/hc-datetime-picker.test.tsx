...
test("should render a date and time range picker ", () => {
  const {getByPlaceholderText, getAllByText} = render(
    <HCDateTimePicker name="hc-datepicker-test" time={true} placeholder={["Start Date Time", "End Date Time"]} />,
  );
  const datePickerElement = screen.getByTestId("hc-datepicker-test");
...
test("should default start date to today's date  ", () => {
  ...
  const activeDate = getAllByText(
    (_content, element) => (element?.classList !== null && element?.classList?.contains("active")) || false,
  );
