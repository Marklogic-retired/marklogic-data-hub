...
const HCInput = React.forwardRef<HTMLInputElement, Props>((props, ref?) => {
  ...
  const handleOnKeyDown = (event, onKeyDown) => {
    if (onKeyDown) {
      onKeyDown(event, inputRefAux);
    }
  };

  useEffect(() => {
    if (props?.value) setMessage(props.value === " " ? "" : props.value);
  }, [props?.value]);
      ...
      <Form.Group className="w-100">
        ...
        >
          ...
          <Form.Control
            ...
            onKeyPress={event =>
              props?.onPressEnter ? (handleKeyPressEnter(event) ? props?.onPressEnter(true) : false) : props?.onKeyPress
            }
            ref={ref}
          ...
          {!props?.allowClear && !props?.disabled && props?.error && (
            <InputGroup.Text
              style={{backgroundColor: "white", padding: props?.suffix ? 2 : ""}}
              className={[props.classNameFull, styles.noBorders].join(" ")}
            >
