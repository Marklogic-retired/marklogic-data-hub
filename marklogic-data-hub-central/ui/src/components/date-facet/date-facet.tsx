...
const DateFacet: React.FC<Props> = props => {
  ...
  const onChange = (startDate, endDate) => {
    ...
    if (dateArray.length && dateArray[0] && startDate.isValid()) {
      ...
      dateArray[0] &&
        dateArray[1] &&
        setDatePickerValue([dayjs(dateArray[0].format("YYYY-MM-DD")), dayjs(dateArray[1].format("YYYY-MM-DD"))]);
    } else {
      props.onChange(
        props.datatype,
        props.constraint,
  ...
  return (
    <div className={styles.name} data-testid="facet-date-picker" id={`${props.name}-facet-date-picker`}>
      <p className={styles.name}>
        <HCTooltip text={props.name.replace(/\./g, " > ")} id={props.name + "-date-tooltip"} placement="top">
          {formatTitle()}
        </HCTooltip>
      </p>
      <HCDateTimePicker
