...
const MonitorFacet: React.FC<Props> = props => {
  ...
  const setCheckedOptions = selectedOptions => {
    ...
    if (facetName) {
      if (monitorOptions.selectedFacets.length === 0) {
        setChecked([]);
      }
  ...
  useEffect(() => {
    if (
      ...
      monitorOptions.selectedFacets.hasOwnProperty(props.name)
    ) {
      setCheckedOptions(monitorOptions);
    } else if (
      Object.entries(monitorGreyedOptions.selectedFacets).length === 0 ||
      !monitorGreyedOptions.selectedFacets.hasOwnProperty(props.name)
    ) {
      setChecked([]);
    }
  }, [monitorOptions]);
    ...
    return (
      <div key={"facet" + index} className={styles.checkContainer}>
        <HCCheckbox
          ...
          tooltip={
            facet?.value.toLowerCase() === "ingestion"
              ? "Loading"
              : facet.value[0].toUpperCase() + facet.value.substring(1)
          }
        />
      </div>
    );
  ...
  return (
    <div
      className={styles.facetContainer}
      style={{"marginLeft": "7px"}}
      data-testid={stringConverter(props.displayName) + "-facet"}
    >
      <div className={styles.header}>
        <div className={styles.name} data-testid={stringConverter(props.displayName) + "-facet"} tabIndex={0}>
          <HCTooltip text={props.displayName} id="display-name-tooltip" placement="top">
            <span>{props.displayName}</span>
          </HCTooltip>
          <span
            tabIndex={0}
            onKeyDown={e => serviceNameKeyDownHandler(e, `${stringConverter(props.displayName)}`)}
            onBlur={e => serviceNameKeyDownHandler(e, `${stringConverter(props.displayName)}`)}
          >
            <HCTooltip
              text={props.tooltip}
              id="info-tooltip"
              placement="top-start"
              show={
                stringConverter(props.displayName) === "status"
                  ? statusTooltipVisible
                    ? statusTooltipVisible
                    : undefined
                  : stringConverter(props.displayName) === "flow"
                    ? flowTooltipVisible
                      ? flowTooltipVisible
                      : undefined
                    : stringConverter(props.displayName) === "step"
                      ? stepTooltipVisible
                        ? stepTooltipVisible
                        : undefined
                      : undefined
              ...
              <span>
                {props.tooltip ? (
                  <FontAwesomeIcon
                    ...
                    icon={faInfoCircle}
                    size="sm"
                    data-testid={`info-tooltip-${props.name}`}
                  />
                ) : (
                  ""
                )}
        ...
        <div className={styles.summary}>
          ...
          >
            {show ? (
              <ChevronDown aria-label="icon: chevron-down" className={styles.toggleIcon} />
            ) : (
              <ChevronRight aria-label="icon: chevron-right" className={styles.toggleIcon} />
            )}
          </div>
        </div>
      </div>
