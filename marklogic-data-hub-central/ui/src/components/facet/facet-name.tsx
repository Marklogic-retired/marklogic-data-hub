...
const FacetName: React.FC<Props> = ({category, name, facet, checked, index, handleClick}) => {
  ...
  return (
    <div className={styles.checkContainer} key={index} data-testid={facet.value} data-cy={stringConverter(name) + "-facet-item"}>
      <HCCheckbox
        id={id}
      ...
      <div
        ...
        data-cy={`${stringConverter(name)}-${facet.value}-count`}
      >
