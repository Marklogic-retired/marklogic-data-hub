        ...
        <div aria-label="add-step-confirmation" className={"mb-4"} style={{fontSize: "16px"}}>
          {isStepInFlow(loadArtifactName, flowName) ? (
            <p aria-label="step-in-flow">
              The step <strong>{loadArtifactName}</strong> is already in the flow <strong>{flowName}</strong>. Would you
              like to add another instance of the step?
            </p>
          ) : (
    ...
    <HCModal show={addExistingStepDialogVisible} onHide={onCancel}>
      ...
      <Modal.Body className={"text-center pt-0 pb-4"}>
        <div className={`mb-4`} style={{fontSize: "16px"}}>
          {
            <p aria-label="step-in-flow">
              The step <b>{loadArtifactName}</b> is already in the flow <b>{flowName}</b>.
            </p>
          }
        </div>
        <div>
          <HCButton
            variant="primary"
            data-testid={`${loadArtifactName}-to-${flowName}-Exists-Confirm`}
            aria-label={"Ok"}
            type="submit"
            className={"me-2"}
            onClick={onConfirmOk}
    ...
    <div id="load-card" aria-label="load-card" className={styles.loadCard}>
      <Row>
        {props.canReadWrite ? (
          ...
          <Col xs={"auto"}>
            <HCTooltip
              id="disabled-load-tooltip"
              placement={"top"}
              text={"Load: " + SecurityTooltips.missingPermission}
              className={styles.tooltipOverlayStyle}
            >
              <span>
                <HCCard className={styles.addNewCardDisabled} data-testid="disabledAddNewCard">
                ...
                >
                  <div className={styles.formatContainer}>
                    <div
                      ...
                      style={sourceFormatStyle(elem.sourceFormat)}
                      aria-label={`${elem.name}-sourceFormat`}
                    >
                      {sourceFormatOptions[elem.sourceFormat].label}
                    </div>
                  </div>
                  ...
                  <div className={styles.cardLinks} style={{display: showLinks === elem.name ? "block" : "none"}}>
                    {props.canWriteFlow ? (
                      ...
                      <div className={styles.cardDisabledLink} data-testid={`${elem.name}-toNewFlow`}>
                        {" "}
                        Add step to a new flow
                      </div>
                    )}
                    ...
                    <div className={styles.cardNonLink} data-testid={`${elem.name}-toExistingFlow`}>
                      ...
                      {selectVisible ? (
                        <HCTooltip
                          id={`${elem.name}missing-permission-tooltip`}
