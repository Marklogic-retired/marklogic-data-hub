...
const BaseEntitiesFacet: React.FC<Props> = ({
  setCurrentBaseEntities,
  setEntitySpecificPanel,
  currentBaseEntities,
  allBaseEntities,
  entityIndicatorData,
  setActiveAccordionRelatedEntities,
  activeKey,
}) => {
  const {searchOptions, setSearchOptions} = useContext(SearchContext);

  const [entityNames, setEntityNames] = useState<string[]>(
    searchOptions.entityTypeIds.length === 0 ? ["All Entities"] : entitiesSorting(searchOptions.entityTypeIds),
  );
  ...
  useEffect(() => {
    const isAllEntities =
      searchOptions.entityTypeIds.length === 0 || searchOptions.entityTypeIds.length === allBaseEntities.length;
    if (isAllEntities) {
      setEntityNames(["All Entities"]);
    }
    if (
      allBaseEntities.length !== 0 &&
      searchOptions.entityTypeIds.length > 0 &&
      searchOptions.entityTypeIds.length !== allBaseEntities.length
    ) {
      const {entityTypeIds} = searchOptions;
      let entitiesFiltered = allBaseEntities
        .map(element => ({value: element.name, label: element.name, isDisabled: false}))
  ...
  const childrenOptions = baseEntitiesSorting(allBaseEntities)
    .map(element => {
  ...
  const handleChange = selection => {
    ...
    if (selectedItems.length === 0 || selectedItems[selectedItems.length - 1] === "All Entities") {
      ...
      if (activeKey.indexOf("related-entities") !== -1) {
        setActiveAccordionRelatedEntities("related-entities");
      }
    } else {
  ...
  return (
    <>
      <Select
        ...
        isClearable={false}
        value={
          entityNames ? entityNames.map(d => ({value: d, label: d})) : [{value: "All Entities", label: "All Entities"}]
        }
        ...
        formatOptionLabel={({value, label}) => {
          ...
          return (
            <HCTooltip
              text={invalidEntities[value] ? ExploreGraphViewToolTips.invalidDefinitionToolTip : ""}
              placement="top"
              id="baseEntityOptionsToolTip"
              key={value}
            >
              <span aria-label={`base-option-${value}`}>{label}</span>
            </HCTooltip>
          );
        }}
        styles={{
      ...
      <div aria-label="base-entities-selection">
        {displayList.map(({name, color, filter, icon, isDefinitionInvalid}) => {
          ...
          if (name) {
            return (
              <HCTooltip
                text={ExploreGraphViewToolTips.entityToolTip}
                placement="top"
                id="baseEntityToolTip"
                key={name}
              >
                <div
                  key={name}
                  ...
                  tabIndex={0}
                  onKeyDown={event => {
                    if (event.key === "Enter") setEntitySpecificPanel(entitySpecificPanelInfo);
                  }}
                >
                  ...
                  <span className={styles.entityAmount} aria-label={`base-entities-${name}-filter`}>
                    <span className={styles.entityAmountFilter}>
                      {isNotEmptyIndicatorData &&
                        entityIndicatorData?.entities &&
                        entityIndicatorData.entities[name]?.filter !== 0 &&
                        showFilter(entityIndicatorData?.entities[name]?.filter)}
                    </span>
                    {isNotEmptyIndicatorData &&
                      entityIndicatorData?.entities &&
                      entityIndicatorData.entities[name]?.amount}
                  </span>
                  {isNotEmptyIndicatorData && (
      ...
      <div
        ...
        onClick={onShowMore}
        data-cy="show-more-base-entities"
        style={{display: currentBaseEntities.length > MINIMUM_ENTITIES ? "block" : "none"}}
      >
        <span
          tabIndex={0}
          onKeyDown={event => {
            if (event.key === "Enter") onShowMore();
          }}
        >
          {showMore ? "<< less" : "more >>"}
        </span>
