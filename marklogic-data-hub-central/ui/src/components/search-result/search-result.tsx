...
const SearchResult: React.FC<Props> = props => {
  ...
  const {searchOptions, setGraphViewOptions, setSavedNode} = useContext(SearchContext);
    ...
    <>
      <div className={"w-100"}>
        <div className={`d-flex align-items-center ${styles.title}`} onClick={() => onExpand()}>
          ...
          <span className={styles.entityName} data-cy="entity-name" data-testid={"entity-name"}>
            {itemEntityName}
          </span>
          {primaryKey && (
            <span data-cy="primary-key" data-testid={"primary-key"} className={styles.primaryKey}>
              {primaryKey}:
            </span>
          )}
          <span data-cy="primary-key-value" aria-label={itemEntityName + "-" + primaryKeyValue}>
            {primaryKeyValue}
          </span>
          <div className={`ms-auto ${styles.redirectIcons}`}>
            ...
            >
              <HCTooltip text="Show the processed data" id="instance-icon-tooltip" placement="top-start">
                <i>
                  <FontAwesomeIcon className={styles.iconHover} icon={faThList} size="sm" data-testid="instance-icon" />
                </i>
              </HCTooltip>
            </Link>
            <Link
            ...
            >
              <HCTooltip
                text={"Show the complete " + recordTypeVal.toUpperCase()}
                id="source-icon-tooltip"
                placement="top-start"
              >
                  ...
                  {canWriteMatchMerge ? (
                    <HCTooltip text={"Unmerge Documents"} id="unmerge-icon-tooltip" placement="top-end">
                      <i>
                        <MdCallSplit
                          className={styles.unMergeIcon}
                          data-testid={`unmerge-icon`}
                          aria-label={`unmerge-icon`}
                          onClick={e => openUnmergeCompare(item, e)}
                        />
                      </i>
                    </HCTooltip>
                  ) : (
        ...
        <div className={styles.metadata}>
          {createdOnVal && (
            <div className={styles.metaItem}>
              ...
              <span className={styles.metaValue} data-cy="created-on" data-testid={"created-on"}>
                {dateConverter(createdOnVal)}
              </span>
            </div>
          )}
          {sourcesVal && (
            <div className={styles.metaItem}>
              ...
              <span className={styles.metaValue} data-cy="sources" data-testid={"sources"}>
                {sourcesVal}
              </span>
            </div>
          )}
          {recordTypeVal && (
            <div className={styles.metaItem}>
              ...
              <span className={styles.format} data-cy="record-type" data-testid={"record-type"}>
                {recordTypeVal}
              </span>
            </div>
          )}
        ...
        <div
          ...
          data-cy="expandable-view"
          aria-label={`expandable-view-${primaryKeyValue}`}
        >
