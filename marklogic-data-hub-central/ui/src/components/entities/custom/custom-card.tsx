    ...
    <HCModal show={addDialogVisible} onHide={onCancel}>
      ...
      <Modal.Body className={"pt-0 pb-4"}>
        <div aria-label="add-step-confirmation" style={{fontSize: "16px"}}>
          {isStepInFlow(stepName, flowName) ? (
            <p aria-label="step-in-flow">
              The step <strong>{stepName}</strong> is already in the flow <strong>{flowName}</strong>. Would you like to
              add another instance?
            </p>
          ) : (
            <p aria-label="step-not-in-flow">
              Are you sure you want to add the step <strong>{stepName}</strong> to the flow <strong>{flowName}</strong>?
            </p>
          )}
        </div>
        <div className={"d-flex justify-content-center pt-4 pb-2"}>
          ...
          <HCButton
            data-testid={`${stepName}-to-${flowName}-Confirm`}
            aria-label={"Yes"}
            variant="primary"
            type="submit"
            onClick={() => onAddOk(stepName, flowName)}
          >
            {"Yes"}
          </HCButton>
        </div>
              ...
              >
                ...
                >
                  <div className={styles.formatFileContainer}>
                    <span aria-label={`${elem.name}-step-label`} className={styles.customNameStyle}>
                      ...
                      Add step to an existing flow
                      {
                        ...
                        selectVisible ? (
                          <HCTooltip
                            ...
                            id="select-flow-tooltip"
                            placement="top"
                            show={tooltipVisible && !props.canWriteFlow}
                          >
