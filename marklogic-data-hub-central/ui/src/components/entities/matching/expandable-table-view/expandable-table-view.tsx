...
const testMatchedUriTableColumns = [
  {
    ...
    width: "40%",
    formatter: (ruleName, key) =>
      ruleName.map(property => {
        return (
          <span className={styles.rulesetColumn} key={key} aria-label={ruleName}>
            {property}
          </span>
        );
      }),
  },
  {
    ...
    width: "6%",
    formatter: (matchedRulesetType, row, index) =>
      row.matchedRulesetType.map(rulesetType => {
        return (
          rulesetType &&
          rulesetType.toLowerCase() === "exact" && (
            <span className={styles.testMatchedColumns} key={row} aria-label={row.matchedRulesetType + " " + index}>
  ...
  },
  {
    ...
    formatter: (matchedRulesetType, row, index) =>
      row.matchedRulesetType.map(rulesetType => {
        return (
          rulesetType &&
          rulesetType.toLowerCase() === "synonym" && (
            <span className={styles.testMatchedColumns} key={row} aria-label={row.matchedRulesetType + " " + index}>
              <FontAwesomeIcon className={styles.checkIcon} icon={faCheck} data-testid={"facet-" + rulesetType} />
            </span>
          )
  ...
  },
  {
    ...
    formatter: (matchedRulesetType, row, index) =>
      row.matchedRulesetType.map(rulesetType => {
        return (
          rulesetType &&
          rulesetType.toLowerCase() === "double metaphone" && (
            <span className={styles.testMatchedColumns} key={row} aria-label={row.matchedRulesetType + " " + index}>
              <FontAwesomeIcon className={styles.checkIcon} icon={faCheck} data-testid={"facet-" + rulesetType} />
            </span>
          )
  ...
  },
  {
    ...
    formatter: (matchedRulesetType, row, index) =>
      row.matchedRulesetType.map(rulesetType => {
        return (
          rulesetType &&
          rulesetType.toLowerCase() === "reduce" && (
            <span className={styles.testMatchedColumns} key={row} aria-label={row.matchedRulesetType + " " + index}>
              <FontAwesomeIcon className={styles.checkIcon} icon={faCheck} data-testid={"facet-" + rulesetType} />
            </span>
          )
  ...
  },
  {
    ...
    formatter: (matchedRulesetType, row, index) =>
      row.matchedRulesetType.map(rulesetType => {
        return (
          rulesetType &&
          rulesetType.toLowerCase() === "custom" && (
            <span className={styles.testMatchedColumns} key={row} aria-label={row.matchedRulesetType + " " + index}>
              <FontAwesomeIcon className={styles.checkIcon} icon={faCheck} data-testid={"facet-" + rulesetType} />
            </span>
          )
  ...
  },
  {
    ...
    width: "15%",
    formatter: (scores, key) => (
      <span key={key} aria-label={"score " + scores.scores[0]}>
        {scores.scores[0] > 0 && (
          <ProgressBar
            now={scores.scores[0]}
            label={scores.matchedRule[0] !== "Reduce" ? `${scores.scores[0]}` : `-${scores.scores[0]}`}
            variant={scores.matchedRule[0] !== "Reduce" ? "success" : "danger"}
          />
        )}
      </span>
    ),
...
const ExpandableTableView: React.FC<Props> = props => {
  ...
  return (
    <div className={styles.expandedTableView}>
      <HCTable
      ...
      />
      <div className={styles.boldTextDisplay}> Total Score: {props.rowData.score}</div>
    </div>
  );
};
