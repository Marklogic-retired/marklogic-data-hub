...
const CompareValuesModal: React.FC<Props> = props => {
  ...
  const {searchOptions, toggleMergeUnmerge} = useContext(SearchContext);
  ...
  const parseDataToTable = (entityDefinitionsArray: Definition[], matchedPropertiesArray, uriInfo) => {
    ...
    let entityTypeDefinition: Definition =
      entityDefinitionsArray.find(definition => definition?.name === activeEntityName) || DEFAULT_ENTITY_DEFINITION;
    ...
    let columns: any[] = [
      {
        ...
        formatter: (text, row) => {
          return (
            <span className={row.hasOwnProperty("children") ? styles.nameColumnStyle : ""} aria-label={text.name}>
              {text.name}
            </span>
          );
        },
      ...
      formatter: (property, key) => {
        ...
        return (
          <span
            key={key}
            aria-label={
              property.value && property.value.length > 0 ? `${property.value}-preview` : "empty-preview-cell"
            }
          >
            {mergedOutput}
          </span>
    ...
    return (
      <div className={styles.paginationContainer}>
        ...
        <div className={styles.paginationLegend}>
          <span>
            <strong>
              {!props.isMerge ? "Preview " : ""} {start}
            </strong>{" "}
            to <strong>{end}</strong> of <strong>{props.uris.length}</strong> Documents
          </span>
        </div>
  ...
  const moreUrisInfo = (
    <Overlay show={showUrisPopover} target={targetUrisPopover} placement="right">
      <Popover
        id={`more-uris-tooltip`}
        className={styles.moreUrisPopover}
      ...
      >
        <Popover.Body className={styles.moreUrisPopover}>
          {props.uriCompared.length < 30 ? (
            <div className={styles.moreUrisInfo} aria-label="more-uri-info">
              All URIs included in this {props.isMerge ? "merge" : "unmerge"} are displayed below (
              <strong>{props.uriCompared.length} total</strong>): <br />
              <br />
              {props.uriCompared.map((uri, index) => {
                return (
                  <div key={index}>
                    <span className={styles.uriText} aria-label={`${uri}-uri`}>
                      {uri}
                    </span>
                    <br />
                  </div>
            ...
            <div>
              <div className={styles.moreUrisInfo} aria-label="more-uri-info-limit">
                The first <strong>30</strong> URIs included in this {props.isMerge ? "merge" : "unmerge"} are displayed
                below (<strong>{props.uriCompared.length} total</strong>): <br />
                <br />
                {props.uriCompared.map((uri, index) => {
                  return index < 30 ? (
                    <div>
  ...
  const mergeUnmergeConfirmation = (
    <HCModal
      ...
      dialogClassName={styles.confirmationModal}
    >
      <Modal.Body>
    ...
    <>
      ...
      >
        <Modal.Header className={"bb-none"}>
          <span className={styles.compareValuesModalHeading} data-testid="compareTitle">
            Compare
          </span>
          {!props.isPreview ? (
        ...
        <Modal.Body>
          <div className={styles.compareValuesModelBodyContainer}>
            <div className="w-100 d-flex">
              <span className={styles.compareValuesEntityName}>
                Entity:{" "}
                <strong>
                  {props.isPreview ? props.activeStepDetails.entityName : props?.activeStepDetails[0]?.name || ""}
                </strong>
              </span>
            </div>
            <div className={styles.paginationRow}>
              <div
                className={props?.isMerge ? styles.paginationRowCleanSpaceMerge : styles.paginationRowCleanSpaceUnmerge}
              />
              <div className={styles.paginationRowActions}>{props.uris.length > pageSize && renderPagination()}</div>
              <div className={props.isMerge ? styles.paginationRowLegendMerge : styles.paginationRowLegendUnmerge}>
                <span className={styles.matchIconContainer}>
                  <img src={backgroundImage} className={styles.matchIcon} />
                </span>
            ...
            <div>
              {columns.length > 0 && (
                <HCTable
                  ...
                  className={
                    props.isMerge
                      ? `compare-values-model ${styles.compareValuesModelTable}`
                      : `compare-values-model-unmerge ${styles.compareValuesModelTable}`
                  }
        ...
        {!props.isPreview ? (
          <Modal.Footer>
            ...
            <HCButton
              variant="primary"
              loading={isLoading}
              aria-label="confirm-merge-unmerge"
