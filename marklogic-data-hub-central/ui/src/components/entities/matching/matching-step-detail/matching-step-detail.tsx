                        ...
                        {combinationsObject.minimumMatchContributions.map(minMatchArray => {
                          return <div>{minMatchArray.map((obj, index, arr) => {
                            if (arr.length - 1 === index) {
                              return <span aria-label={`rulesetName-${combinationsObject.name}-${obj.rulesetName}`}>{obj.rulesetName}</span>
                            } else {
                              return <span aria-label={`rulesetName-${combinationsObject.name}-${obj.rulesetName}`}>{obj.rulesetName} <span className={styles.period}></span> </span>
                            }
                          })}</div>
