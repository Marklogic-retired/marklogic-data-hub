    ...
    <>
      <div className={styles.closeEntityIconsSidebar}>
        <HCTooltip id="reference-tooltip" text="Return to the main side panel." placement="top">
          <ChevronDoubleRight
            aria-label="base-entity-icons-list-close"
            className={styles.chevronBack}
            onClick={closeSpecificSidebar}
            tabIndex={0}
            onKeyDown={event => handleKeyDownMain(event)}
          />
        </HCTooltip>
      </div>
      <div className={styles.iconsContainer}>
        <div className={styles.entityIconList} aria-label="base-entity-icons-list">
          {currentBaseEntities.map(
            ({color, icon, name}, index) =>
              name && (
                <div
                  tabIndex={0}
                  key={name}
        ...
        {currentRelatedEntitiesArray.length > 0 && (
          <div
            className={!graphView ? styles.relatedEntityIconListDisabled : styles.relatedEntityIconList}
            aria-label="related-entity-icons-list"
          >
            {currentRelatedEntitiesArray.map(
              ({color, icon, name}, index) =>
                name && (
                  <HCTooltip
                    key={index}
                  ...
                  >
                    <div
                      key={name}
                      aria-label={`related-entity-icon-${name}`}
                      style={{backgroundColor: color}}
                      className={!graphView ? styles.entityIconListItemDisabled : styles.entityIconListItem}
                      onClick={e => (!graphView ? e.preventDefault() : handleRelatedEntityClicked(index))}
                    >
                      {icon ? <DynamicIcons name={icon} /> : <DynamicIcons name={defaultIcon} />}
                    </div>
                  </HCTooltip>
