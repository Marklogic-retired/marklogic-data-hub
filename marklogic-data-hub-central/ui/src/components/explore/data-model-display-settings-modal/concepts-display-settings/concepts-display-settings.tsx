  ...
  const exploreSettingsColumns: any = [
    {
      ...
      formatter: (text, row) => {
        return (
          <span aria-label={`${row.concept}-conceptName`}>
            <Highlighter
        ...
        return (
          <div className="text-center">
            <EntityTypeColorPicker
              color={row.color}
              entityType={row.rowKey}
              handleColorChange={(color, event) =>
                onConceptsColumnValueChange(row, {color, event}, ConceptsTableColumns.Color)
              }
            />
          </div>
        );
      ...
      formatter: (text, row) => {
        return (
          <div
            ...
            aria-label={`${row.concept}-icon-picker`}
            id={`${row.rowKey}-icon-picker`}
            data-icon={row.icon}
          >
            <HCIconPicker
              value={row.icon}
              onChange={value => onConceptsColumnValueChange(row, value, ConceptsTableColumns.Icon)}
            />
          </div>
        );
      },
  ...
  return (
    <div className={styles.conceptsTable}>
      <HCTable
        ...
        component={"property"}
        nestedParams={{
          headerColumns: exploreSettingsColumns,
          iconCellList: [],
          state: [tableExpandedKeys, setTableExpandedKeys],
        }}
      />
    </div>
  );
