...
import {faCheckCircle, faClock, faInfoCircle, faBan} from "@fortawesome/free-solid-svg-icons";
...
const Flows: React.FC<Props> = ({
  ...
  const getUserPreferencesLS = () => {
    ...
    if (sessionUser) {
      ...
      if (userPreferences) {
        ...
        //if (Object.keys(oldOptions.selectedStepsDataUser?.selectedStepOptions).length === 0) return false
      } else { return false; }
    }
  ...
  const lastRunResponse = (step) => {
    ...
    } else if (step.lastRunStatus === "canceled") {
      return (
        <span>
