...
const Membership: React.FC<Props> = (props) => {
  ...
  const getItems = () => {
    ...
    const items = config?.lists?.map((list, index) => {
      ...
      const important = membership?.important === "true";
      ...
      return (
        <div key={index} className={classes} style={membershipStyle}>
          ...
          {membership ? <span className="date"><DateTime config={{"format": props.config.dateFormat}}>{membership.ts}</DateTime></span> : <span className="empty date"></span>}
