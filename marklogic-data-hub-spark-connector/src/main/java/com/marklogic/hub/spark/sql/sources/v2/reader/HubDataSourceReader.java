...
/**
 * Responsible for calling the export initialization endpoint which allows it to know how many input partitions to
...
public class HubDataSourceReader extends LoggingObject implements DataSourceReader {
    ...
    public HubDataSourceReader(DataSourceOptions dataSourceOptions, PartitionCountProvider partitionCountProvider) {
        ...
        this.initializationResponse = initialize(partitionCountProvider);
        this.schema = (StructType) StructType.fromJson(initializationResponse.get("schema").toString());
    }

    private JsonNode initialize(PartitionCountProvider partitionCountProvider) {
    ...
    public List<InputPartition<InternalRow>> planInputPartitions() {
        ...
        initializationResponse.get("partitions").forEach(partition -> {
            inputPartitions.add(new HubInputPartition(this.options, partition, this.schema));
        });
        logger.info("Input partition count: " + inputPartitions.size());
