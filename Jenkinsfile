...
pipeline{
    ...
    stage('rh7_cluster_9.0-9'){
      ...
      def slurper = new JsonSlurperClassic().parseText(commitMessage.toString().trim())
        ...
        JIRA_ID=JIRA_ID.split(" ")[0];
        commitMessage=null;
        //jiraAddComment comment: 'Jenkins rh7_cluster_9.0-9 Test Results For PR Available', idOrKey: JIRA_ID, site: 'JIRA'
        }
      }
      post{
        always{
            sh 'rm -rf $WORKSPACE/xdmp'
                  ...
                  success {
                    ...
                    sendMail Email,'Check the Pipeline View Here: ${JENKINS_URL}/blue/organizations/jenkins/Datahub_CI/detail/$JOB_BASE_NAME/$BUILD_ID  \n\n\n Check Console Output Here: ${BUILD_URL}/console \n\n\n All the End to End tests on rh7 cluster 9.0-9 of the branch $BRANCH_NAME passed and the next stage is to merge it to release branch if all the end-end tests pass',false,'rh7_cluster_9.0-9 Tests for $BRANCH_NAME Passed'
                   }
                   failure {
