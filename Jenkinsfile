       ...
       /* stage('cypresse2e-firefox') {
        ...
        steps {
           timeout(time: 3,  unit: 'HOURS'){
             catchError(buildResult: 'SUCCESS',catchInterruptions: true, stageResult: 'UNSTABLE') { RTLTests('Release','10.0-9.5') }
        }}
        post{
           success {
        ...
        stage('rh7_cluster_10.0-10'){
         ...
         stage('rh7_cluster_10.0-9.5'){
             ...
             steps{
              timeout(time: 5,  unit: 'HOURS'){
               catchError(buildResult: 'SUCCESS', catchInterruptions: true, stageResult: 'FAILURE'){dhflinuxTests("10.0-9.5","Release")}
             }}
             post{
                 always{
                     sh 'rm -rf $WORKSPACE/xdmp'
                           ...
                           success {
                             println("rh7_cluster_10.0-9.5 Tests Completed")
                             sendMail Email,'<h3>Tests Passed on  10.0-9.5 ML Server Cluster </h3><h4><a href=${RUN_DISPLAY_URL}>Check the Pipeline View</a></h4><h4> <a href=${BUILD_URL}/console> Check Console Output Here</a></h4>',false,'$BRANCH_NAME branch | Linux RH7 | ML-10.0-9.5 | Cluster | Passed'
                            }
                            unstable {
                               println("rh7_cluster_10.0-9.5 Tests Failed")
                               sendMail Email,'<h3>Some Tests Failed on 10.0-9.5 ML Server Cluster </h3><h4><a href=${JENKINS_URL}/blue/organizations/jenkins/Datahub_CI/detail/$JOB_BASE_NAME/$BUILD_ID/tests><font color=red>Check the Test Report</font></a></h4><h4><a href=${RUN_DISPLAY_URL}>Check the Pipeline View</a></h4><h4> <a href=${BUILD_URL}/console> Check Console Output Here</a></h4><h4>Please create bugs for the failed regressions and fix them</h4>',false,'$BRANCH_NAME branch | Linux RH7 | ML-10.0-9.5 | Cluster | Failed'
                           }
                           }
             }
	...
	stage('example projects parallel'){
        		...
        		stage('w12_cluster_10.0-9.5'){
        			...
        			post{
        				always{
                          ...
                          success {
                            println("w12_cluster_10.0-9.5 Tests Completed")
                            sendMail Email,'<h3>Tests Passed on Released 10.0 ML Server Cluster on Windows Platform</h3><h4><a href=${RUN_DISPLAY_URL}>Check the Pipeline View</a></h4><h4> <a href=${BUILD_URL}/console> Check Console Output Here</a></h4>',false,'$BRANCH_NAME branch | Windows W2k12 | ML-10.0-9.5 | Cluster | Passed'
                           }
                           unstable {
        ...
        stage('HC on Prem parallel'){
            ...
            parallel{
                ...
                stage('10.0-9.5-cypress-linux-setup-win-firefox'){
                    agent {label 'dhfLinuxAgent'}
                    steps{
                        script{
                                cypressSetup('Release','10.0-9.5')
                ...
                stage('10.0-9.5-cypress-linux-setup-win-chrome'){
                    agent {label 'dhfLinuxAgent'}
                    steps{
                        script{
                                cypressSetup('Release','10.0-9.5')
                ...
                stage('10.0-9.5-cypress-linux-setup-mac-chrome'){
                    agent {label 'dhfLinuxAgent'}
                    steps{
                        script{
                                cypressSetup('Release','10.0-9.5')
