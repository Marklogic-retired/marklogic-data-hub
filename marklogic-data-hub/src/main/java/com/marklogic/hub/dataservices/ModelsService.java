...
public interface ModelsService {
    ...
    static ModelsService on(DatabaseClient db, JSONWriteHandle serviceDeclaration) {
        final class ModelsServiceImpl implements ModelsService {
            ...
            @Override
            public void updateModelInfo(String name, String description) {
                updateModelInfo(
                    this.req_updateModelInfo.on(this.dbClient), name, description
                    );
            }
            private void updateModelInfo(BaseProxy.DBFunctionRequest request, String name, String description) {
              request
                      .withParams(
            ...
            @Override
            public void updateModelEntityTypes(String name, com.fasterxml.jackson.databind.JsonNode input) {
                updateModelEntityTypes(
                    this.req_updateModelEntityTypes.on(this.dbClient), name, input
                    );
            }
            private void updateModelEntityTypes(BaseProxy.DBFunctionRequest request, String name, com.fasterxml.jackson.databind.JsonNode input) {
              request
                      .withParams(
                          BaseProxy.atomicParam("name", false, BaseProxy.StringType.fromString(name)),
                          BaseProxy.documentParam("input", false, BaseProxy.JsonDocumentType.fromJsonNode(input))
   ...
   * 
   */
    void updateModelInfo(String name, String description);
   ...
   * 
   */
    void updateModelEntityTypes(String name, com.fasterxml.jackson.databind.JsonNode input);
