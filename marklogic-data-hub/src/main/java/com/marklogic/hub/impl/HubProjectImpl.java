            ...
            for (File communityModelFile : communityModelFiles) {
                try {
                    ...
                    for (Iterator<JsonNode> it = modelEdges.elements(); it.hasNext(); ) {
                        ...
                        JsonNode modelFromNode = entityModels.get(modelFromName);
                        String modelToName = edge.path("to").asText();
                        JsonNode modelToNode = modelNodes.path(modelToName);
                        if (!(modelFromNode == null || modelToNode == null)) {
                            ...
                            String modelToTitle = modelToNode.path("entityName").asText();
                            ...
                            if ("concept".equals(modelToNode.path("type").asText())) {
                                ...
                                String modelToBaseUri = modelToNode.path("baseUri").asText();
                                String modelToVersion = modelToNode.path("version").asText();
                                String modelToIRI = modelToBaseUri + modelToTitle + "-" + modelToVersion;
                                String typeToIRI = modelToIRI + "/" + modelToTitle;

                                JsonNode modelFromPropertyNode = modelFromTypeNode.path("properties").path(modelFromProperty);
