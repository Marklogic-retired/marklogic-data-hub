...
public class DataHubImpl implements DataHub {
    ...
    public boolean upgradeHub(List<String> updatedFlows) throws CantUpgradeException {
        ...
        if(alreadyInitialized) {
            try {
                FileUtils.moveDirectory(project.getHubConfigDir().toFile(), project.getProjectDir().resolve(HubProject.HUB_CONFIG_DIR+"-"+gradleVersion).toFile());
                FileUtils.moveDirectory(project.getUserConfigDir().toFile(), project.getProjectDir().resolve(HubProject.USER_CONFIG_DIR+"-"+gradleVersion).toFile());
            }
            catch (IOException e) {
                throw new CantUpgradeException("Unable to backup the configuration directories", e);
            }
        }
        hubConfig.initHubProject();
