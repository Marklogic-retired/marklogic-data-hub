...
public class CreatedByStepFixer extends LoggingObject {
    ...
    private void runFixScript(DatabaseClient contentClient) {
        OutputEndpoint.BulkOutputCaller bulkCaller = OutputEndpoint.on(
            contentClient,
            hubClient.getModulesClient().newJSONDocumentManager().read("/data-hub/5/data-services/bulk/fixCreatedByStep.api", new JacksonHandle())
        ).bulkCaller();
        ...
        bulkCaller.setOutputListener(response -> {
        });
