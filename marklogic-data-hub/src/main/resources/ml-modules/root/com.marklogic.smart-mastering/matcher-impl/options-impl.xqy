...
declare function opt-impl:compile-match-options(
  ...
  if (map:contains($_cached-compiled-match-options, $cache-id)) then
    ...
    let $queries :=
      for $rule-set in $match-rulesets/(*:reduce|*:add|*:expand|self::matchRulesets)
      let $weight := fn:number($rule-set/(@weight|weight))
      let $is-complex-rule := fn:local-name-from-QName(fn:node-name($rule-set)) eq "matchRulesets"
      let $match-rules :=
      ...
      let $match-queries :=
            ...
            let $type := if ($is-complex-rule) then
                ...
                fn:local-name-from-QName(fn:node-name($match-rule))
            let $full-property-name := $match-rule/(@property-name|propertyName|entityPropertyPath|documentXPath)
            let $match-value-function :=
              if (map:contains($property-names-to-values, fn:string($full-property-name))) then
                ...
                let $local-name := fn:head((fn:local-name-from-QName(fn:node-name($full-property-name)), $type))
                let $match-value-function :=
                  switch ($local-name)
                  case "property-name"
                  ...
                  case "entityPropertyPath" return
                    function($document) {
                      if (fn:contains($full-property-name,".")) then
                        xdmp:unpath("/*/" || fn:replace($full-property-name,"\.","/*/*:"), $xpath-namespaces, $document/*:envelope/*:instance)
                      else
                        let $qname := fn:QName($target-entity-def/namespaceUri, $full-property-name)
                        return $document/*:envelope/*:instance/*/*[fn:node-name(.) eq $qname]
                    }
    ...
    let $minimum-threshold-combinations :=
        opt-impl:minimum-threshold-combinations($queries, $minimum-threshold)
