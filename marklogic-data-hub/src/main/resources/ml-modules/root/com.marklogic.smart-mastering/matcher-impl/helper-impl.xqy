    ...
    if (map:contains($_cached-property-name-to-queries, $key)) then
      ...
      let $helper-query :=
        if (fn:exists($property-info)) then
          ...
          let $property-entity-qname := fn:QName($namespace, helper-impl:NCName-compatible($property-info => map:get("entityTitle")))
          let $scope-query :=
            function($values, $is-json) {
              if ($is-json) then
                cts:json-property-scope-query(fn:string($property-entity-qname), $values)
              else
                cts:element-query($property-entity-qname, $values)
            }
          let $index-reference := $property-info => map:get("indexReference")
          return
            if (fn:exists($index-reference)) then
