...
function main(content, options) {
  ...
  for (uriToProcess of theseURIsToProcess) {
    xdmp.log("uriToProcess: " + uriToProcess);
    ...
    if (uriActionDetails && uriActionDetails.action === 'merge') {
      ...
      if (otherMergesForURI.some((otherMerge) => otherMerge.uris.length > uriActionDetails.uris.length)) {
        xdmp.log("there is a matchSummary with more URIs");
        continue;
      }
    ...
    xdmp.log("before buildContentObjectsFromMatchSummary");
    xdmp.log("uriToProcess: " + uriToProcess);
    xdmp.log(thisMatchSummary);
    ...
    xdmp.log("results.length: " + results.length);

    applyPermissionsFromOptions(results, options);
    returnArray = returnArray.concat(results);
  }

  xdmp.log("returnArray.length: " + returnArray.length);
