...
module namespace stepValidate = "http://marklogic.com/rest-api/extensions/stepValidate";
...
declare private variable $extName := "stepValidate";
declare private variable $modPath := "/data-hub/5/extensions/stepValidate.sjs";
declare private variable $caller  := xdmp:function(
  xs:QName("applyOnce"), "/data-hub/5/rest-api/lib/extensions-util.sjs"
);

declare function stepValidate:source-format() as xs:string {
  "javascript"
};
declare %rapi:transaction-mode("query") function stepValidate:get(
  $context as map:map, $params as map:map
) as map:map {
  xdmp:apply($caller,$extName,$modPath,"GET",$context,$params)
};
declare %rapi:transaction-mode("query") function stepValidate:delete(
  $context as map:map, $params as map:map
) as map:map {
  xdmp:apply($caller,$extName,$modPath,"DELETE",$context,$params)
};
declare %rapi:transaction-mode("query") function stepValidate:post(
  $context as map:map, $params as map:map, $input as document-node()*
) as map:map {
  xdmp:apply($caller,$extName,$modPath,"POST",$context,$params,$input)
};
declare %rapi:transaction-mode("query") function stepValidate:put($context as map:map, $params as map:map, $input as document-node()*
) as map:map {
  xdmp:apply($caller,$extName,$modPath,"PUT",$context,$params,$input)
};
declare function stepValidate:transform(
  $context as map:map, $params as map:map, $input as document-node()?
) as map:map {
