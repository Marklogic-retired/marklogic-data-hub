...
function getJobDocWithId(jobId, throwErrorIfMissing) {
  ...
  if (jobDocsByUri.has(jobUri)) {
    return jobDocsByUri.get(jobUri);
  }
  ...
  if (jobDoc) {
    jobDocsByUri.set(jobUri, jobDoc);
  }
  return jobDoc;
}
...
function getJobDocs(status) {
  ...
  hubUtils.invokeFunction(function() {
    let uris = cts.uris("", ["concurrent", "score-zero"], cts.andQuery(query));
    for (let doc of uris) {
...
function getJobDocsByFlow(flowName) {
  return hubUtils.invokeFunction(function() {
    ...
    let jobDoc = cts.search(cts.andQuery(query), ["score-zero", "unfaceted"], 0);
    if (jobDoc) {
  ...
  if (stepResponse.stepDefinitionName && stepResponse.stepDefinitionType) {
    const stepDefinition = fn.head(hubUtils.invokeFunction(function () {
      return new StepDefinition()
        .getStepDefinitionByNameAndType(stepResponse.stepDefinitionName, stepResponse.stepDefinitionType);
    }, config.FINALDATABASE
    ));
...
function findProvenanceRecordUriFromJobId(jobId, targetDatabase) {
  ...
  const provRecordUri = fn.head(
    hubUtils.invokeFunction(function () {
      return cts.uris("", ["limit=1", "concurrent", "score-zero"], jobProvQuery);
    }, targetDatabase)
  );
...
/**
   ...
   * @returns
   */
export function updateJob(jobDoc) {
  ...
  if (jobDocsByUri.has(jobUri)) {
    Object.assign(jobDocsByUri.get(jobUri), jobDoc);
  }
