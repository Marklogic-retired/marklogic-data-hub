    ...
    let $inputs           := (
        ...
        return
            ...
            then xdmp:set($default-meta,
                docmodupdtform:prepare-metadata($curr-uri,$content-format,$part)
...
) as map:map?
{
    if (empty($content-disposition)) then ()
        ...
        return (
            ...
            for $token in (
                cts:tokenize($content-disposition, "zxx"),
                if (ends-with($content-disposition,";")) then ()
                else cts:tokenize(";", "zxx")
                )
            return (
                switch($goal)
                case $NAME return
...
) as map:map
{
    ...
    then docmodupdtform:prepare-metadata-json($uri,xdmp:from-json($metadata-doc))
...
) as map:map
{
    ...
    let $properties  := docmodupdtform:prepare-properties-json(
        $uri, docmodupdtform:get-map-sequence($input,"properties")
        )
...
) as xs:string?
{
    if (empty($type)) then ()
        ...
        let $tokens     := cts:tokenize($type, "zxx")
