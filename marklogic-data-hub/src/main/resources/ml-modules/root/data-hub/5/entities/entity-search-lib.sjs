...
// If this is defined in user config, its value is stored here to avoid repeated lookups of this value
// from user config via the getEntityDetails function
let entityDetailsModulePath = null;
 ...
 * the location of those parts can be avoided. By default, this function assumes that the document conforms to the
 * ES envelope format. A user can override this via the "modeling/entityDetailsModulePath" property in DHF user config.
 *
...
function getEntityDetails(doc) {
  const detailsModulePath = getEntityDetailsModulePathFromUserConfig();
  if (detailsModulePath !== null) {
    xdmp.trace("hub-entity", "Invoking entity details module: " + detailsModulePath);
    return fn.head(xdmp.invoke(detailsModulePath, {doc: doc}));
  }
...
function getEntityDetailsModulePathFromUserConfig() {
  if (entityDetailsModulePath === null || entityDetailsModulePath === undefined) {
    console.log("checking for path");
    entityDetailsModulePath = entityLib.getEntityDetailsModulePath();
    if (entityDetailsModulePath === undefined || entityDetailsModulePath === null) {
      // If no path was set, then set it to "" for the scope of this transaction so that subsequent calls to
      // this function do not try to retrieve this value again
      entityDetailsModulePath = "";
      return null;
    }
  }
  return entityDetailsModulePath;
}
