...
if (xdmp.uriContentType(uri) === "application/json") {
  try {
    ...
    if (artifactType === "step" && artifact) {
      artifactType = String(artifact.stepDefinitionType).toLowerCase();
    }
    invokeFeatureMethods(artifactType, artifactName, uri, artifact, "onArtifactSave");
  } catch (e) {
    const stackTrace = xdmp.toJsonString(e.stackFrames || e.stack);
    const errMsg = `Error running onArtifactSave on features at URI: ${uri}; cause: ${e.toString()}`;
    httpUtils.throwBadRequestWithArray([errMsg, stackTrace]);
  }
}