...
let allUrisResults = pma.previewMatchingActivity(options, sourceQuery, allUris, 0);
...
let uris_0_1_Results = pma.previewMatchingActivity(options, sourceQuery, uris0_1, 0);
...
let uris_2_3_Results = pma.previewMatchingActivity(options, sourceQuery, uris2_3, 0);
...
results.push(test.assertEqual(5, uris_2_3_Results.actionPreview.length, "There should be 5 matching pairs for match on docs 2 and 3"));
results.push(test.assertEqual("7.5", uris_2_3_Results.actionPreview[0].score.toString(), "For match with docs 2 and 3, the first score should be 7.5 for the match between 2 and 3"));
results.push(test.assertEqual("9.5", uris_2_3_Results.actionPreview[1].score.toString(), "For match with docs 2 and 3, the 2nd score should be 9.5"));
...
let sampleResults = pma.previewMatchingActivity(options, sourceQuery, [], 3);
