...
function testFuzzyMatches() {
  ...
  let customAction = resultsNode.xpath('matchSummary/actionDetails/*[action = "customActions"]');
  let notifyAction = resultsNode.xpath('matchSummary/actionDetails/*[action = "notify"]');
  let customActionObj = fn.exists(customAction) ? fn.head(customAction).toObject(): {matchResults: []};
  return [
    test.assertEqual(1, fn.count(results), `A matchSummary should be returned. Results: ${results}`),
    test.assertEqual(0, fn.count(mergeAction), `Zero merge actions should be in the matchSummary. Results: ${xdmp.toJsonString(results)}`),
    test.assertEqual(1, fn.count(customAction), `One custom action should be in the matchSummary. Results: ${xdmp.toJsonString(results)}`),
    test.assertEqual(0, fn.count(notifyAction), `One notify action should be in the matchSummary. Results: ${xdmp.toJsonString(results)}`),
    test.assertTrue(customActionObj.matchResults.some(match => match.matchedRulesets && match.matchedRulesets.some(ruleset => ruleset.rulesetName === "reduce for household")), `Custom action should have reduce be in the matchSummary. Results: ${xdmp.toJsonString(results)}`)
  ];
}
