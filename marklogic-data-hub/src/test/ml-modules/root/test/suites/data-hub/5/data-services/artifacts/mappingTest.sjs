...
function updateMappingConfig(artifactName) {
    const result = invokeSetService('mapping', artifactName, {'name': `${artifactName}`, 'targetEntityType': 'TestEntity-hasMappingConfig', 'description': 'Mapping does ...', 'selectedSource': 'query', 'sourceQuery': '', 'collections': ['RAW-COL']});
    return [
...
function createMappingWithSameNameButDifferentEntityType(artifactName) {
  try {
    invokeSetService('mapping', artifactName, {'name': `${artifactName}`, 'targetEntityType': 'SomeOtherEntity-hasMappingConfig', 'selectedSource': 'query'});
