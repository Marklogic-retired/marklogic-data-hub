...
function runXMLMapping(){
  ...
  const content = ["/content/customerInfo.xml"].map(uri => {
    return {
      uri: uri,
      value: cts.doc(uri)
    };
  ...
  let results = datahub.flow.runFlow('customerFlow', 'test-job', content, {outputFormat: 'xml', mapping:{name:'mapCustomersXML'}}, 2);
