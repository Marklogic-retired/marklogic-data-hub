...
public class Installer {
    ...
    public static void main(String[] args) {
        ...
        try {
            ctx.getBean(HubConfigInterceptor.class).claimHubConfig(Thread.currentThread().getName());

            HubConfigImpl hubConfig = ctx.getBean(HubConfigImpl.class);

            ctx.getBean(DataHub.class).install();

            loadTestModules(hubConfig, false);
     ...
     */
    private static void addStatusPrivilegeToDataHubDeveloper(HubConfig hubConfig) {
    ...
    public static void loadTestModules(HubConfig hubConfig, boolean includeSecurityCommands) {
        ...
        try {
            ...
            new SimpleAppDeployer(commands.toArray(new Command[0])).deploy(appConfig);
