...
public class FileUtil {
	...
	public static String createFolderIfNecessary(String path, String folderName) {
		...
		if(!rootDirectory.exists()) {
			LOGGER.debug("New folder is created at " + rootDirectory.getAbsolutePath());
			rootDirectory.mkdir();
		}
		if(rootDirectory.exists() && rootDirectory.isDirectory()) {
			File folder = new File(rootDirectory.getAbsolutePath() + File.separator + folderName);
			if(!folder.exists()) {
				folder.mkdir();
				LOGGER.debug("New folder is created at "+ folder.getAbsolutePath());
			}
		}
