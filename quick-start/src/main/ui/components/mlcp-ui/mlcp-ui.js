...
@Injectable()
@Inject('$scope', '$mdDialog', '$mdToast', 'flow', 'mlcpOptions')
export class MlcpUi {
  mlcp = {};

  constructor($scope, $mdDialog, $mdToast, flow, mlcpOptions) {
    this.$scope = $scope;
    ...
    this.$scope.$watch('ctrl.mlcp', () => {
      this.updateMlcpCommand();
    }, true);

    this.$scope.$watch('ctrl.groups', () => {
      this.updateMlcpCommand();
    }, true);

    this.groups = this.groups(flow.entityName, flow.flowName, this.mlcp);
  }

  /* eslint-disable max-len */
  groups(entityName, flowName, previousOptions) {
    const groups = [
      {
  ...
  buildMlcpOptions() {
    ...
    const inputFilePath = this.mlcp.input_file_path;
    // const inputFileType = this.mlcp.input_file_type;

    this.mlcp = {};
    this.addMlcpOption(options, 'import', null, false);
    this.addMlcpOption(options, 'mode', 'local', false);
    this.addMlcpOption(options, 'input_file_path', inputFilePath, true);
    this.addMlcpOption(options, 'output_uri_replace', '"' + inputFilePath + ',\'\'"', true);
  ...
  folderClicked($event) {
    ...
    this.mlcp.input_file_path = $event.detail;
  }

  cmdCopied() {
    const simple = this.$mdToast.simple()
      .textContent('Command copied to the clipboard.')
      .hideDelay(3000);

    simple._options.parent = jQuery('.mlcp-cmd');
    this.$mdToast.show(simple);
  }

  runImport() {
    this.$mdDialog.hide(this.mlcp);
  }
