...
export class SeedConfig {
   ...
   * The default port is `5555`, which can be overriden by the  `--port`
   ...
   * The default environment is `dev`, which can be overriden by the
   * `--config-env ENV_NAME` flag when running `npm start`.
   */
  ENV = getEnvironment();

  /**
   * The flag for the debug option of the application.
   * The default value is `false`, which can be overriden by the `--debug`
   * flag when running `npm start`.
   * @type {boolean}
   */
  ...
  */
  ...
  /**
   * The build interval which will force the TypeScript compiler to perform
   * a typed compile run. Between the typed runs, a typeless compile is run,
   * which is typically much faster. For example, if set to 5, the initial
   * compile will be typed, followed by 5 typeless runs, then another typed
   * run, and so on. If a compile error is encountered, the build will use
   * typed compilation until the error is resolved. The default value is `0`,
   * meaning typed compilation will always be performed.
   * @type {number}
   */
   ...
   * @type {string}
   */
  APP_CLIENT = argv['client'] || 'main/ui';

  /**
   * The bootstrap file to be used to boot the application. The file to be
   * used is dependent if the hot-loader option is used or not.
   * Per default (non hot-loader mode) the `main.ts` file will be used, with
   * the hot-loader option enabled, the `hot_loader_main.ts` file will be used.
   * @type {string}
   */
  BOOTSTRAP_MODULE =
    this.BOOTSTRAP_DIR + (this.ENABLE_HOT_LOADING ? 'hot_loader_main' : 'main');
   ...
   * @type {string}
   */
  APP_TITLE = 'Data Hub Quick Start';
   ...
   * @type {string}
   */
  APP_DEST = `${this.DIST_DIR}`;

  /**
   * The folder for the index.html file
   * @type {string}
   */
  INDEX_HTML_DEST = 'src/main/resources/templates'
